...
class Slip_remun extends Controller
{
    public function download($data){
        $customKey = '@RSig2024';
        try {
            $decryptedData = Crypt::decryptString($data, $customKey);
            $idKey = explode('|',$decryptedData);
            $data = [];
            $data["profil"] = DB::table("pencairan_jasa as pj")
                            ...
                            ->where([
                                "pj.id_header"    => $idKey[0],
                                "e.emp_id"        => $idKey[1],
                            ])->first();
            $folderPath = 'public/slip_remun/' . str_replace('/','_',$data["profil"]->no_pencairan);
            ...
            if (file_exists($pdfPath)) {
                ...
                $dataJasa = $this->get_detail_jasa($data["profil"]->emp_id,$data["profil"]->id_cair_header);
                $dataJasa["profil"] = $data["profil"];
                // Jika belum ada, buat PDF baru
                $pdf = PDF::loadview("laporan.v_rekap_jaspel",$dataJasa);
    ...
    function get_detail_jasa($emp_id,$idHeader) {
        ...
        $data['jasa_by_penjamin'] = DB::select("
            SELECT x.nama_komponen as pelayanan,x.nama_penjamin,x.skor_jasa,(x.skor_jasa/x.skor*x.nominal_terima)jasa_tunai FROM (
                ...
                WHERE jm.emp_id = '$emp_id' AND jp.id_cair = $idHeader
